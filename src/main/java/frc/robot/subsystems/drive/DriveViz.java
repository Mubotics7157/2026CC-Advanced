...
public class DriveViz {
    ...
    public void telemeterize(Pose2d pose, double odometryPeriod, SwerveModuleState[] moduleStates) {
        if (pose == null || moduleStates == null || moduleStates.length < 4) {
            return;
        }
        ...
        Logger.recordOutput("Drive/Viz/OdomPeriod", odometryPeriod);
        Logger.recordOutput("Drive/Viz/ModuleStatesCurrent", moduleStates);

        /* Telemeterize the module's states */
        for (int i = 0; i < 4; ++i) {
            moduleSpeeds[i].setAngle(moduleStates[i].angle);
            moduleDirections[i].setAngle(moduleStates[i].angle);
            moduleSpeeds[i].setLength(moduleStates[i].speedMetersPerSecond / (2 * MaxSpeed));
        }
    }
}
