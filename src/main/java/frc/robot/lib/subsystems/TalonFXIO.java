...
public class TalonFXIO implements MotorIO {
    ...
    private Follower followerControl = new Follower(0, MotorAlignmentValue.Aligned);
    ...
    public void follow(CANDeviceId masterId, boolean opposeMasterDirection) {
        followerControl =
                new Follower(
                        masterId.getDeviceNumber(),
                        opposeMasterDirection
                                ? MotorAlignmentValue.Opposed
                                : MotorAlignmentValue.Aligned);
        CTREUtil.tryUntilOK(
                () -> talon.setControl(followerControl), this.config.talonCANID.getDeviceNumber());
    }
